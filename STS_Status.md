...
## In Progress
- Manually rerun the previously failing real-device project update / scanned-receipt persistence flow on the updated build and capture fresh logs so the exact mutation path can be checked against the new CloudKit upsert, duplicate-receipt normalization, and pre-save scanned-receipt field-edit hardening.
