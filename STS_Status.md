...
## In Progress
- Manually rerun the previously failing real-device scanned-receipt flow on the updated build and capture fresh logs so the new scanner launch-state fix can be checked against the repeated camera re-entry and onboarding-prompt regressions from `rheirlogs7.md`.
...
- This environment can install, launch, and inspect the physical-device build, but the exact user-driven scanned-receipt flow still needs a manual rerun to confirm the camera no longer reopens after capture, the compact launcher does not regress into the onboarding prompt after dismissal, and cancel teardown no longer surfaces scanner errors on hardware.

## Latest Evidence
- `ReceiptScannerView.swift` now runs one-time scanner initialization through `ReceiptScannerPresentationState.performInitialSetup(...)`, preserves scanner state across repeated view appearances, and routes hidden-intro returns through a compact `.launcher` entry state instead of reopening the full onboarding prompt.
- `RHEIRTests.swift` now covers the new scanner launch-state behavior directly, proving queued cancellation does not reopen the sheet, hidden-intro returns land on `.launcher`, and repeated initial setup calls do not auto-start a second scanner session.
- Focused scanner lifecycle parity: `xcodebuild -project /Users/kevinbarrett/Dev/RHEIR/RHEIR.xcodeproj -scheme RHEIR -destination 'platform=iOS Simulator,id=DD0211FE-8732-4DA9-9E9E-78C61F0734DC' -derivedDataPath /tmp/rheir_phase2_scanner_reentry_targeted_dd -resultBundlePath /tmp/rheir_phase2_scanner_reentry_targeted.xcresult test -only-testing:RHEIRTests/ReceiptScannerLifecycleTests` -> PASS (`/tmp/rheir_phase2_scanner_reentry_targeted.xcresult`, `5 tests in 1 suite`)
- Gate A: `xcodebuild -project /Users/kevinbarrett/Dev/RHEIR/RHEIR.xcodeproj -scheme RHEIR -destination 'generic/platform=iOS Simulator' -derivedDataPath /tmp/rheir_gateA_phase2_scanner_reentry_dd -resultBundlePath /tmp/rheir_gateA_phase2_scanner_reentry.xcresult clean build` -> PASS (`/tmp/rheir_gateA_phase2_scanner_reentry.xcresult`)
