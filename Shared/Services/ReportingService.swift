...
public class ReportingService: ObservableObject {
    ...
    public func generateReceiptsCSV(project: Project) -> Data? {
        var csvContent = "Date,Vendor,Receipt Number,Payment Method,Receipt Total,Receipt Tax,Receipt Discount,Tip Amount,Price Per Gallon,Top-Level Category,Receipt Subcategory,Line Item,Quantity,Unit Price,Line Amount,Line Category,Line Subcategory,Notes,Is Return,Source Receipt ID\n"
    ...
    private func receiptCSVRow(
        ...
        [
            ...
            receipt.tipAmount.map { String(format: "%.2f", $0) } ?? "",
            receipt.pricePerGallon.map { String(format: "%.3f", $0) } ?? "",
