...
struct DataManagementSection: View {
    ...
    var body: some View {
        Section(header: Text("Data Management & Debug")) {
            ...
            if projectViewModel.hasProjectsNeedingMigration() {
    ...
    private func checkDataStatus() {
        Task {
            let status = await projectViewModel.getDataStatus()
            await MainActor.run {
    ...
    private func emergencyRecoverData() {
        Task {
            let recovered = await projectViewModel.emergencyRecoverFromBackup()
            await MainActor.run {
                environmentInfo = recovered ? "✅ Successfully recovered data from backup" : "❌ No backup data found"
                showingEnvironmentAlert = true
            }
        }
    }
    ...
    private func testCloudKitEnvironment() {
        // FIX: Use completion callback correctly
        projectViewModel.getDetailedCloudKitStatus { info in
    ...
    private func debugLocalDataAction() {
        // FIX: Use completion callback correctly
        projectViewModel.getDiagnosticInfo { info in
            environmentInfo = info
            showingEnvironmentAlert = true
        }
    }
    ...
    private func exploreCloudKitDataAction() {
        Task {
            let info = await projectViewModel.listCloudKitProjects()
            await MainActor.run {
                environmentInfo = """
                CloudKit Project Explorer:
    ...
    private func checkOfflineCapabilities() {
        Task {
            let status = await projectViewModel.getOfflineStatus()
            await MainActor.run {
                environmentInfo = status
                showingEnvironmentAlert = true
            }
    ...
    private func triggerManualSync() {
        Task {
            let result = await projectViewModel.triggerManualSync()
            await MainActor.run {
                environmentInfo = """
                ...
                Check offline capabilities for sync progress.
                """
                showingEnvironmentAlert = true
            }
        }
    ...
    private func migratePhotosToCloudKit() {
        guard let firstProject = projectViewModel.organizationProjects.first(where: { 
            // Find a project that needs migration
            projectViewModel.getMigrationStats(project: $0).totalPhotos > 0
        }) else {
            environmentInfo = """
            No Photo Migration Needed
            
            All projects are already using the new photo system.
            No legacy imageDatas found to migrate.
            """
            showingEnvironmentAlert = true
            return
        }
        ...
        Task {
            do {
                ...
                await MainActor.run {
                    ...
                    Successfully migrated project: \(migratedProject.name)
                    
                    Photos are now stored as CKAssets with automatic:
                ...
                await MainActor.run {
                    ...
                    Please check CloudKit connection and try again.
                    """
                    showingEnvironmentAlert = true
                }
            }
    ...
    private func getPhotoMigrationStats() {
        var totalProjects = 0
        var totalPhotos = 0
        var needsMigrationCount = 0
        
        for project in projectViewModel.organizationProjects {
            let stats = projectViewModel.getMigrationStats(project: project)
        ...
        Projects with Photos: \(totalProjects)
        Total Photos: \(totalPhotos)
        Projects Needing Migration: \(needsMigrationCount)
        
        Status: \(projectViewModel.hasProjectsNeedingMigration() ? "⚠️ Migration needed" : "✅ Migration complete")
        """
        showingEnvironmentAlert = true
    }
}