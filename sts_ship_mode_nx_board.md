...
## Open Work
- Finish replacing raw `print(...)` tracing in `ProjectDataMigrationService.swift` and the other non-hardened production seams.
- Continue shrinking the remaining oversized active state owners.
- After the `SimpleCloudKitSharingService.swift` checkpoint commit, continue the `ProjectDataMigrationService.swift` cleanup slice.
