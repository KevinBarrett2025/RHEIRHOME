...
- `ReceiptScannerView.swift` now queues document-scanner results until the VisionKit sheet fully dismisses and ignores duplicate document-camera terminal callbacks so scanner teardown no longer competes with OCR/AI state changes.
...
- Stable simulator evidence is green on the staged checkpoint:
  - Focused scanner lifecycle parity: PASS (`xcodebuild -project /Users/kevinbarrett/Dev/RHEIR/RHEIR.xcodeproj -scheme RHEIR -destination 'platform=iOS Simulator,id=DD0211FE-8732-4DA9-9E9E-78C61F0734DC' -derivedDataPath /tmp/rheir_phase2_scanner_lifecycle_targeted_dd -resultBundlePath /tmp/rheir_phase2_scanner_lifecycle_targeted.xcresult test -only-testing:RHEIRTests/ReceiptScannerLifecycleTests`, `/tmp/rheir_phase2_scanner_lifecycle_targeted.xcresult`, `2 tests in 1 suite`)
  - Gate A CLI clean build: PASS (`xcodebuild -project /Users/kevinbarrett/Dev/RHEIR/RHEIR.xcodeproj -scheme RHEIR -destination 'generic/platform=iOS Simulator' -derivedDataPath /tmp/rheir_gateA_phase2_scanner_lifecycle_dd -resultBundlePath /tmp/rheir_gateA_phase2_scanner_lifecycle.xcresult clean build`, `/tmp/rheir_gateA_phase2_scanner_lifecycle.xcresult`)
...
- Manually rerun the previously failing real-device scanned-receipt flow on the updated build, verify the first scan no longer destabilizes, verify the saved scanned receipt still exists after leaving and returning, and capture fresh logs for the exact mutation path.
...
## Blockers
- Physical-device build/signing and startup/on-disk payload compaction are now validated, and simulator parity is green for CloudKit upsert, duplicate-receipt normalization, persisted-edit behavior, and the scanner queued-dismiss lifecycle, but the exact first-scan hardware receipt flow still needs a manual rerun with fresh logs.
