...
- `ReceiptEditView.swift` now exposes contractor-native row-swipe refund actions with tax-inclusive confirmation, reversible refunded-row actions in the same edit workflow, and linked financial-history locking from silent rewrites, while `ReceiptDetailView.swift` keeps linked refund balances visible without forcing a separate receipt-level refund sheet.
- `ReceiptEditView.swift` now stages row-swipe refund and revert actions inside the edit sheet, keeping attempted ledger changes local until `Save` and discarding them on `Cancel`.
...
- `ReceiptDetailView.swift` now exposes confirmed reversal for linked refund receipts, and `ReceiptsView.swift` now filters by refund state plus payment/card with gross-purchase, refunded, and net summary cards.
...
- `LandingPageView` now exposes a fast-ship Business Resources hub from Projects, and `BudgetBreakdownView` exposes the same hub from active-project analytics so workers/rates, vendors, and payment methods are reachable without Company/Organization admin UI.
- `RHEIRUITests.swift` now proves the Business Resources hub opens and the Workers & Rates management surface is real instead of a placeholder.
- `LaborModuleView` now uses the Business Resources worker/rate seam directly, keeps the Labor surface top-aligned, exposes worker management from the Labor header, and routes worker rows into the real payment ledger.
- `WorkHour` now persists `LaborPaymentEntry` records, deriving paid/unpaid balances from payment entries so partial and split payments can be represented without losing the original labor cost.
- `ProjectViewModel+TimeEntry` now records and reverses labor payments through the shared project mutation seam so Labor totals, selected project state, local persistence, and sync scheduling refresh together.
- `LaborPaymentView` is now included in the app target and supports unpaid/paid/summary tabs, partial payment amounts, payment method/reference capture, paid entry reference display, and unpay/reissue reversal actions.
- `RHEIRTests` and `RHEIRUITests` now prove the Labor payment ledger path with partial payment persistence, reversal accounting, worker row rate metadata, payment ledger navigation, and paid-entry reference visibility.
- `BusinessWorkersResourceView` now routes add/edit into the compiled enhanced team-member editor, restoring multi-rate job/rate management for office, demo, plumbing, and other contractor work types instead of flattening workers to one default rate.
...
- `BusinessWorkerRosterCache` now reconciles transient partial worker snapshots after add, `LogHoursView` initializes a valid lunch interval before save, `LaborPaymentView` exposes direct worker-ledger `Add Hours`, and owner placeholder rows now surface a repair path instead of remaining anonymous forever.
...
- `BudgetBreakdownView` now opens a live Reports & Exports sheet from the active project analytics area with project PDF, job-cost CSV, receipt line-item CSV, labor payment ledger CSV, labor timesheet CSV, and task closeout CSV preview/export actions.
- `ReportingService` is now compiled into the app target and produces contractor-facing report artifacts from persisted project data instead of placeholder report rows.
- PDF previews now render as swipeable, pinch-to-zoom pages, while CSV previews render inspectable table rows before export.
...
- Stable simulator evidence is green on the staged checkpoint:
  - Tasks before/after photo-proof Gate A: PASS (`xcodebuild -project /Users/kevinbarrett/Dev/RHEIR/RHEIR.xcodeproj -scheme RHEIR -destination 'generic/platform=iOS Simulator' -resultBundlePath /tmp/rheir_gateA_tasks_before_after_photos_final2.xcresult clean build`, `/tmp/rheir_gateA_tasks_before_after_photos_final2.xcresult`)
  - Tasks before/after photo-proof focused parity: PASS (`xcodebuild -project /Users/kevinbarrett/Dev/RHEIR/RHEIR.xcodeproj -scheme RHEIR -destination 'platform=iOS Simulator,id=DD0211FE-8732-4DA9-9E9E-78C61F0734DC' -resultBundlePath /tmp/rheir_tasks_before_after_photos_parity_final2.xcresult test '-only-testing:RHEIRUITests/RHEIRUITests/testTaskManagementSupportsAssignmentEditingCompletionProofAndRefresh()' '-only-testing:RHEIRUITests/RHEIRUITests/testTaskManagementSupportsCreateEditReopenAndDelete()' -only-testing:RHEIRTests/ProjectMutationPropagationTests`, `/tmp/rheir_tasks_before_after_photos_parity_final2.xcresult`, `2 UI tests plus 4 mutation tests`)
  - Tasks before/after photo-proof visual evidence: kept xcresult attachments `Task before photo controls` and `Task completion photo proof sheet`
  - Tasks before/after photo-proof pbxproj drift: NONE
  - Reports Gate A: PASS (`xcodebuild -project /Users/kevinbarrett/Dev/RHEIR/RHEIR.xcodeproj -scheme RHEIR -destination 'generic/platform=iOS Simulator' -derivedDataPath /tmp/rheir_gateA_reports_dd -resultBundlePath /tmp/rheir_gateA_reports.xcresult clean build`, `/tmp/rheir_gateA_reports.xcresult`)
  - Reports focused parity: PASS (`xcodebuild -project /Users/kevinbarrett/Dev/RHEIR/RHEIR.xcodeproj -scheme RHEIR -destination 'platform=iOS Simulator,id=DD0211FE-8732-4DA9-9E9E-78C61F0734DC' -derivedDataPath /tmp/rheir_reports_parity_final5_dd -resultBundlePath /tmp/rheir_reports_parity_final5.xcresult test '-only-testing:RHEIRTests/ReportingServiceTests' '-only-testing:RHEIRUITests/RHEIRUITests/testProjectReportsPreviewSurfaceExposesPDFAndCSVExports()'`, `/tmp/rheir_reports_parity_final5.xcresult`, `3 report service tests plus 1 UI preview/export test`)
  - Reports visual evidence: kept xcresult attachment `Project PDF preview`
  - Reports pbxproj drift: INTENTIONAL (`Shared/Services/ReportingService.swift` added to the compiled app target)
  ...
  - Business Resources focused parity: PASS (`xcodebuild -project /Users/kevinbarrett/Dev/RHEIR/RHEIR.xcodeproj -scheme RHEIR -destination 'platform=iOS Simulator,id=DD0211FE-8732-4DA9-9E9E-78C61F0734DC' -derivedDataPath /tmp/rheir_business_resources_parity_dd -resultBundlePath /tmp/rheir_business_resources_parity.xcresult test -only-testing:RHEIRUITests/RHEIRUITests/testBusinessResourcesEntryOpensReusableResourceHub`, `/tmp/rheir_business_resources_parity.xcresult`, `1 UI test`)
  - Business Resources Gate A: PASS (`xcodebuild -project /Users/kevinbarrett/Dev/RHEIR/RHEIR.xcodeproj -scheme RHEIR -destination 'generic/platform=iOS Simulator' -derivedDataPath /tmp/rheir_gateA_business_resources_dd -resultBundlePath /tmp/rheir_gateA_business_resources.xcresult clean build`, `/tmp/rheir_gateA_business_resources.xcresult`)
  - Business Resources visual evidence: Projects home entry (`/var/folders/jw/s9ytdthj0nj1jwccblxc1jmh0000gn/T/screenshot_optimized_4b1b8a6d-d7a7-410f-9dbf-88bc359593bf.jpg`) and hub (`/var/folders/jw/s9ytdthj0nj1jwccblxc1jmh0000gn/T/screenshot_optimized_f8dc70de-9299-42ec-819d-9529c9c9771c.jpg`)
  - Labor follow-up focused parity: PASS (`/tmp/rheir_labor_icandidate_parity_20260518.log`, worker-roster suite plus worker-ledger UI path; `/tmp/rheir_labor_icandidate_unit_parity_serial_20260518.log`, `23 tests across 3 suites`)
  - Labor follow-up Gate A: PASS (`/tmp/rheir_gateA_labor_followup_20260518.xcresult`)
  - Labor payment focused parity: PASS (`xcodebuild -project /Users/kevinbarrett/Dev/RHEIR/RHEIR.xcodeproj -scheme RHEIR -destination 'platform=iOS Simulator,name=STS iPhone 17 Phase017,OS=26.2' -derivedDataPath /tmp/rheir_labor_payment_parity_dd -resultBundlePath /tmp/rheir_labor_payment_parity_final_rerun.xcresult -only-testing:RHEIRTests/LaborPaymentLedgerTests -only-testing:RHEIRUITests/RHEIRUITests/testLaborManagementOpensWorkerPaymentLedger test`, `/tmp/rheir_labor_payment_parity_final_rerun.xcresult`, `2 unit tests plus 1 UI test`)
  - Labor payment Gate A: PASS (`xcodebuild -project /Users/kevinbarrett/Dev/RHEIR/RHEIR.xcodeproj -scheme RHEIR -destination 'generic/platform=iOS Simulator' -derivedDataPath /tmp/rheir_gateA_labor_payment_dd -resultBundlePath /tmp/rheir_gateA_labor_payment_rerun.xcresult clean build`, `/tmp/rheir_gateA_labor_payment_rerun.xcresult`)
  - Labor payment pbxproj drift: INTENTIONAL (`Shared/Features/Labor/LaborPaymentView.swift` added to the compiled app target)
...
- Re-run the same-user iCloud hardware pass with the latest Labor follow-up first: verify immediate worker refresh after add, lunch-deducted hours, worker-ledger add-hours, owner-profile repair, and offline-to-online persistence, then decide whether iCloud restore/sync is safe to promise at launch.
...
- Receipt edit-item revert Gate A: PASS (`/tmp/rheir_gateA_receipt_edit_item_revert_20260517_2.xcresult`; sandbox attempt `_1` failed on CoreSimulator asset-catalog runtime, escalated rerun passed)
- Receipt edit-item revert focused parity: PASS (`/tmp/rheir_receipt_edit_item_revert_parity_20260517_3.xcresult`, `2 refund unit tests plus 1 edit-sheet refund/revert UI relaunch test`; sandbox attempt `_2` failed before test execution on CoreSimulator service loss)
- Receipt edit-item revert visual evidence: kept xcresult attachment `Receipt refund filters and reversal after relaunch`
- Receipt edit-item revert pbxproj drift: NONE
- Receipt refund reversal/filter Gate A: PASS (`/tmp/rheir_gateA_receipt_refund_reverse_filter_20260517_2.xcresult`; sandbox attempt `_1` failed on CoreSimulator asset-catalog runtime, escalated rerun passed)
- Receipt refund reversal/filter focused parity: PASS (`/tmp/rheir_receipt_refund_reverse_filter_parity_20260517_2.xcresult`, `2 refund unit tests plus 1 refund filter/reversal UI relaunch test`; sandbox attempt `_1` failed before build on CoreSimulator service loss)
...
- Receipt linked-refund presentation Gate A: PASS (`/tmp/rheir_gateA_receipt_linked_refund_child_20260517_1.xcresult`)
