...
## Open Work
- Finish replacing raw `print(...)` tracing in the other non-hardened production seams, starting with `Shared/Features/TeamMembers/EnhancedTeamMemberDetailView.swift`.
- Continue shrinking the remaining oversized active state owners.
- After the `Shared/Features/TeamMembers/EnhancedTeamMemberDetailView.swift` checkpoint, continue the next highest-value non-hardened seam.
