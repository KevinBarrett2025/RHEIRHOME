...
- `ReceiptScannerView.swift` now performs one-time scanner initialization, preserves scanner state across repeated view appearances, and routes hidden-intro returns through a compact launcher state so a successful first scan cannot immediately relaunch VisionKit or regress back into the onboarding prompt.
...
- Stable simulator evidence is green on the staged checkpoint:
  - Focused scanner lifecycle parity: PASS (`xcodebuild -project /Users/kevinbarrett/Dev/RHEIR/RHEIR.xcodeproj -scheme RHEIR -destination 'platform=iOS Simulator,id=DD0211FE-8732-4DA9-9E9E-78C61F0734DC' -derivedDataPath /tmp/rheir_phase2_scanner_reentry_targeted_dd -resultBundlePath /tmp/rheir_phase2_scanner_reentry_targeted.xcresult test -only-testing:RHEIRTests/ReceiptScannerLifecycleTests`, `/tmp/rheir_phase2_scanner_reentry_targeted.xcresult`, `5 tests in 1 suite`)
  - Gate A CLI clean build: PASS (`xcodebuild -project /Users/kevinbarrett/Dev/RHEIR/RHEIR.xcodeproj -scheme RHEIR -destination 'generic/platform=iOS Simulator' -derivedDataPath /tmp/rheir_gateA_phase2_scanner_reentry_dd -resultBundlePath /tmp/rheir_gateA_phase2_scanner_reentry.xcresult clean build`, `/tmp/rheir_gateA_phase2_scanner_reentry.xcresult`)
...
## Blockers
- Physical-device build/signing and startup/on-disk payload compaction are now validated, and simulator parity is green for CloudKit upsert, duplicate-receipt normalization, persisted-edit behavior, and the scanner launch-state lifecycle, but the exact first-scan hardware receipt flow still needs a manual rerun with fresh logs.
