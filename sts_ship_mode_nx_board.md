...
- The next deterministic UI harness seam is the manual-entry payment-method picker route.
- Stable simulator evidence is green on the staged checkpoint:
  - Gate A `build_sim`: PASS (`mcp__xcodebuildmcp__build_sim` with `-derivedDataPath /tmp/rheir_gateA_phase1cf_mcp_dd`)
...
- The stable simulator path is now warning-clean, and deterministic signed-out, ready-state, organization-selection, project-selection, labor, company-admin, receipts-entry, and manual-entry vendor-picker coverage are in place; the next release-hardening seam is the deterministic payment-method picker route in `RHEIRUITests`.
