...
- The next deterministic UI harness seam is persisted receipt-edit mutation coverage in seeded selected-project mode.
...
- The stable simulator path is now warning-clean, and deterministic signed-out, ready-state, organization-selection, project-selection, labor, company-admin, receipts-entry, manual-entry vendor-picker, manual-entry add-vendor, manual-entry payment-method-picker, manual-entry add-payment-method, manual-entry submission, saved receipt detail, and receipt-detail action coverage are in place; the next release-hardening seam is persisted receipt-edit mutation coverage in `RHEIRUITests`.
