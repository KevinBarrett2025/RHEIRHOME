...
## Open Work
- Finish replacing raw `print(...)` tracing in the other non-hardened production seams, starting with `Shared/Services/CloudKitPhotoService.swift`.
- Continue shrinking the remaining oversized active state owners.
- After the `Shared/Services/CloudKitPhotoService.swift` checkpoint, continue the next highest-value non-hardened seam.
