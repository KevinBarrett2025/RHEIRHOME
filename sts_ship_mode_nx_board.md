...
- The next deterministic UI harness seam is project selection, using a seeded active organization with multiple active projects and no selected project.
...
- The stable simulator path is now warning-clean, and deterministic signed-out, ready-state, and organization-selection shell coverage are in place; the next release-hardening seam is deterministic project-selection coverage in `RHEIRUITests`, followed by deeper project-context workflow coverage.
