...
## Blockers
- The restored-session freeze and nil-org sign-out replay seam are now both clean in device logs; the remaining launch-critical device risk is the selected-project receipt flow itself on real hardware, specifically confirming immediate launch-time receipt visibility, scanned-item persistence, and menu-only detail actions on device.
