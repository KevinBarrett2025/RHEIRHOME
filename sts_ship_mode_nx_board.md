...
- The next deterministic UI harness seam is receipt-detail edit/delete affordances in seeded selected-project mode.
...
- The stable simulator path is now warning-clean, and deterministic signed-out, ready-state, organization-selection, project-selection, labor, company-admin, receipts-entry, manual-entry vendor-picker, manual-entry add-vendor, manual-entry payment-method-picker, manual-entry add-payment-method, manual-entry submission, and saved receipt detail coverage are in place; the next release-hardening seam is receipt-detail edit/delete affordances in `RHEIRUITests`.
