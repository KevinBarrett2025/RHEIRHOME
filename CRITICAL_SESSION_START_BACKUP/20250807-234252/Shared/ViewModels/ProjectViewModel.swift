...
class ProjectViewModel: ObservableObject {
    ...
    func testCloudKitZoneSetup() async -> String {
        ...
        do {
            ...
            testReport += " Test failed with error: \(error.localizedDescription)\n"
        }
    ...
    func forceRecreateCloudKitZone() async -> String {
        ...
        do {
            ...
            do {
                ...
                report += "   Zone deletion failed (may not exist): \(error.localizedDescription)\n"
            }
            ...
            report += " Zone recreation failed: \(error.localizedDescription)\n"
        }
    ...
    func manuallyCreateFreshZone() async -> String {
        ...
        do {
            ...
            let existingZones = try await privateDB.allRecordZones()
            report += "   Found \(existingZones.count) existing zones\n"
            
            let zoneName = "org-shared-\(organizationID)"
            let zoneID = CKRecordZone.ID(zoneName: zoneName)
            
            if let existingZone = existingZones.first(where: { $0.zoneID.zoneName == zoneName }) {
                report += "   Zone already exists: \(zoneName)\n"
                
                // Check if it has sharing
                let hasShare = try await checkZoneHasShare(existingZone, container: container)
                report += "   Zone has share: \(hasShare ? "" : "")\n"
            ...
            report += "\n MANUAL ZONE CREATION FAILED: \(error.localizedDescription)\n"
        }
    ...
    func migrateLaborHoursToTeamMemberIDs() {
        ...
        if migratedCount > 0 {
            ...
            print("✅ Migration completed: \(migratedCount) work hours migrated to team member IDs")
        } else {
            print("ℹ️ No work hours needed migration")
        }
    }
    ...
    func fixTeamMemberProjectAssignments() {
        ...
        for i in 0..<organizationProjects.count {
            var project = organizationProjects[i]
            let originalAssignmentCount = project.assignedUserIDs.count
        ...
        if updatedCount > 0 {
            saveLocalBackup()
            print("🔧 Fixed assignments for \(updatedCount) projects")
        } else {
            print("ℹ️ All project assignments are already correct")
        }
    }
    ...
    /// Unassign a team member from a specific project
    func unassignTeamMemberFromProject(_ memberID: String, projectID: String) {
        guard let projectIndex = organizationProjects.firstIndex(where: { $0.id.uuidString == projectID }) else {
            print("❌ Project not found for unassignment: \(projectID)")
            return
        }
        ...
        if project.assignedUserIDs.contains(memberID) {
            project.removeUserAssignment(memberID)
            organizationProjects[projectIndex] = project
            
            saveLocalBackup()
            
            print("✅ Unassigned team member \(memberID.prefix(8))... from project \(project.name)")
        } else {
            print("ℹ️ Team member \(memberID.prefix(8))... was not assigned to project \(project.name)")
        }
    }
}