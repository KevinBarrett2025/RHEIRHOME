...
class AuthViewModel: ObservableObject {
    ...
    /// Force recreate CloudKit zones (for debugging)
    func forceRecreateCloudKitZones() async -> String {
        guard let currentOrgID = currentOrg?.id else {
            return "❌ No organization selected for zone recreation"
        }
        
        var report = "🔄 FORCE RECREATING CLOUDKIT ZONES\n"
        report += "=====================================\n\n"
        report += "Organization: \(currentOrgID.prefix(8))...\n"
        report += "Zone recreation temporarily disabled while CloudKit issues are being resolved.\n"
        report += "All data is safely stored locally.\n\n"
        report += "✅ No action needed - local storage is working properly"
