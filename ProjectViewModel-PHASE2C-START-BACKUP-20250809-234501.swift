...
final class ProjectViewModel: ObservableObject {
    ...
    // MARK: - Enterprise Intelligence Access Methods (Simplified)
    ...
    func getNewProjectSuggestions() -> String? {
        ...
        let topVendors = _vendorService.vendorsSortedByName.prefix(5).map { $0.name }
        let topPaymentMethods = _paymentMethodService.paymentMethodsSortedByName.prefix(3).map { $0.displayName }
        
        return """
        🚀 NEW PROJECT SUGGESTIONS
        
        🏪 Recommended Vendors (Top 5 by Usage):
        \(topVendors.isEmpty ? "• No vendor data available yet" : topVendors.enumerated().map { "• \($0.offset + 1). \($0.element)" }.joined(separator: "\n"))
        
        💳 Preferred Payment Methods:
        \(topPaymentMethods.isEmpty ? "• No payment method data available yet" : topPaymentMethods.enumerated().map { "• \($0.offset + 1). \($0.element)" }.joined(separator: "\n"))
        
        💡 Tip: Start adding receipts to build your organizational intelligence!
        """
    }
    ...
    func getAnnualBusinessReport(year: Int? = nil) async -> String? {
        ...
        • Vendor Partners: \(_vendorService.vendorsSortedByName.count)
        • Payment Methods Used: \(_paymentMethodService.paymentMethodsSortedByName.count)
