...
class ProjectViewModel: ObservableObject {
    ...
    @Published private(set) var userProjectAssignments: [String] = []
    ...
    func setupCloudKitZoneForOrganization(_ organizationID: String) async {
        ...
        guard accountStatus == .available else {
            let errorMsg = "iCloud account not available: \(accountStatus ?? .couldNotDetermine)"
        ...
        if isUsingCloudKitForOrganizationData == false {
    ...
    private func checkZoneHasShare(_ zone: CKRecordZone, container: CKContainer) async -> Bool? {
        let privateDB = container.privateCloudDatabase
        ...
        if let rootRecord = rootRecord, let share = rootRecord.share {
    ...
    private func createShareForExistingZone(_ zone: CKRecordZone, organizationID: String, container: CKContainer) async {
        let privateDB = container.privateCloudDatabase
        ...
        if let existingRootRecord = existingRootRecord {
    ...
    private func loadOrganizationProjectsFromCloudKit(zoneID: CKRecordZone.ID) async {
        ...
        let (matchResults, _) = (try? await privateDB.records(matching: query, inZoneWith: zoneID)) ?? ([:], nil)
        ...
        if let cloudKitProjects = cloudKitProjects {
    ...
    func addTask(_ task: ProjectTask) {
        guard let selectedProject = selectedProject,
              let index = organizationProjects.firstIndex(where: { $0.id == selectedProject.id }) else {
            print(" No selected project for task addition")
            return
        }
        
        organizationProjects[index].tasks.append(task)
        self.selectedProject = organizationProjects[index]
        
        saveLocalBackup()
        print(" Added task to \(selectedProject.name)")
    }
    
    func updateTask(_ task: ProjectTask) {
        guard let selectedProject = selectedProject,
              let projectIndex = organizationProjects.firstIndex(where: { $0.id == selectedProject.id }),
              let taskIndex = organizationProjects[projectIndex].tasks.firstIndex(where: { $0.id == task.id }) else {
            print(" No selected project or task for update")
            return
        }
        
        organizationProjects[projectIndex].tasks[taskIndex] = task
        self.selectedProject = organizationProjects[projectIndex]
        
        saveLocalBackup()
        print(" Updated task in \(selectedProject.name)")
    }
    
    func deleteTask(_ task: ProjectTask) {
        guard let selectedProject = selectedProject,
              let index = organizationProjects.firstIndex(where: { $0.id == selectedProject.id }) else {
            print(" No selected project for task deletion")
            return
        }
        
        organizationProjects[index].tasks.removeAll { $0.id == task.id }
        self.selectedProject = organizationProjects[index]
        
        saveLocalBackup()
        print(" Deleted task from \(selectedProject.name)")
    }
    ...
    func addTeamMemberToOrganization(_ teamMember: TeamMember) {
        // Set organization ID if not already set
        var newTeamMember = teamMember
        if let currentOrgID = currentOrganizationID {
            newTeamMember.organizationID = currentOrgID
        }
        
        // Check for duplicates by ID first, then by name
        if teamMembers.contains(where: { $0.id == newTeamMember.id }) {
            print(" Team member with ID \(newTeamMember.id) already exists in organization")
            return
        }
        
        if let currentOrgID = currentOrganizationID,
           teamMembers.contains(where: { $0.name.lowercased() == newTeamMember.name.lowercased() && $0.organizationID == currentOrgID }) {
            print(" Team member with name '\(newTeamMember.name)' already exists in this organization")
            return
        }
        
        // Ensure team member has at least one rate
        if newTeamMember.rates.isEmpty {
            newTeamMember.rates.append(EmployeeRate(taskType: "General Labor", rate: 25.0))
        }
        
        teamMembers.append(newTeamMember)
        print(" Added team member to organization: \(newTeamMember.name)")
    ...
    /// Update a team member in the organization
    func updateTeamMemberInOrganization(_ teamMember: TeamMember) {
        guard let index = teamMembers.firstIndex(where: { $0.id == teamMember.id }) else {
            print(" Team member not found for update: \(teamMember.name)")
            return
        }
    ...
    /// Remove a team member from the organization by ID
    func removeTeamMemberFromOrganization(_ teamMemberID: UUID) {
        teamMembers.removeAll { $0.id == teamMemberID }
    ...
    func emergencyDataRecovery() async -> String {
        ...
        let success = await emergencyRecoverFromBackup()
        
        if success {
            return "Emergency recovery completed - recovered \(organizationProjects.count) projects"
        } else {
            return "Emergency recovery failed - no backup data found"
        }
    }
    
    func triggerManualSync() async -> Bool {
        isBulkSyncing = true
        bulkSyncProgress = "Syncing projects to local storage..."
        defer { 
            isBulkSyncing = false
            bulkSyncProgress = ""
        }
    ...
    func listCloudKitProjects() async -> String {
        var projectList = " LOCAL STORAGE PROJECTS:\n\n"
        ...
        if organizationProjects.isEmpty {
            projectList += "No projects found in local storage"
        }
        
        return projectList
    }
    
    // MARK: - CloudKit Methods (Disabled)
    ...
    // MARK: - Team Collaboration Methods (RE-ENABLED)
    ...
    func getOrganizationCloudKitShareURL() async -> String? {
        ...
        if let rootRecord = rootRecord, let shareReference = rootRecord.share {
            // Fetch the actual share record
            let shareRecord = try? await privateDB.record(for: shareReference.recordID)
            if let shareRecord = shareRecord, let share = shareRecord as? CKShare,
    ...
    func acceptCloudKitOrganizationInvitation(from url: URL) async throws {
        ...
        let acceptedShare = try? await CKContainer.default().accept(metadata)
        ...
        if let zoneID = acceptedShare?.recordID.zoneID {
            await loadOrganizationProjectsFromCloudKit(zoneID: zoneID);
        }
    }
    ...
    func getCloudKitZoneDiagnostics() async -> String {
        ...
        if let zone = orgZone {
            ...
            if let rootRecord = rootRecord {
                ...
                diagnostics += "Root Record: Not found\n"
            }
        } else {
    ...
    /// Test method to check CloudKit zone setup and sharing
    func testCloudKitZoneSetup() async -> String {
        guard let organizationID = currentOrganizationID else {
            return " No organization ID set for testing"
        }
        
        var testReport = " CLOUDKIT ZONE SETUP TEST:\n\n"
        
        do {
            // Test zone setup
            testReport += "1. Setting up CloudKit zone...\n"
            await setupCloudKitZoneForOrganization(organizationID)
            
            testReport += "   Zone setup completed: \(isUsingCloudKitForOrganizationData ? "" : "")\n"
            
            if let error = zoneSetupError {
                testReport += "   Error: \(error)\n"
            }
            
            // Test zone diagnostics
            testReport += "\n2. Running zone diagnostics...\n"
            let diagnostics = await getCloudKitZoneDiagnostics()
            testReport += diagnostics
            
            // Test share URL generation
            testReport += "\n3. Testing share URL generation...\n"
            let shareURL = await getOrganizationCloudKitShareURL()
            if let shareURL = shareURL {
                testReport += "   Share URL: Generated successfully\n"
                testReport += "   URL: \(shareURL)\n"
            } else {
                testReport += "   Share URL: Could not generate\n"
            }
            
            testReport += "\n TEST COMPLETE\n"
            
            // Check if zone is ready for collaboration
            let isCollaborationReady = isUsingCloudKitForOrganizationData
            testReport += "Zone ready for collaboration: \(isCollaborationReady ? "" : "")\n"
    ...
    /// Force recreate the CloudKit zone (for testing)
    func forceRecreateCloudKitZone() async -> String {
        guard let organizationID = currentOrganizationID else {
            return " No organization ID set"
        }
        
        var report = " FORCE RECREATING CLOUDKIT ZONE:\n\n"
        ...
        // Step 1: Try to delete existing zone
        report += "1. Attempting to delete existing zone...\n"
        try? await privateDB.deleteRecordZone(withID: zoneID)
        report += "   Deleted existing zone\n"
        
        // Step 2: Wait a moment for CloudKit to process
        try? await Task.sleep(nanoseconds: 2_000_000_000) // 2 seconds
        
        // Step 3: Create fresh zone
        report += "\n2. Creating fresh zone...\n"
        await setupCloudKitZoneForOrganization(organizationID)
        report += "   Zone creation: \(isUsingCloudKitForOrganizationData ? "" : "")\n"
        ...
        report += "\n3. Testing new zone...\n"
        let testResult = await testCloudKitZoneSetup()
        report += testResult
        
        return report
    }
    
    /// Manually create a fresh CloudKit zone for the current organization (for testing)
    func manuallyCreateFreshZone() async -> String {
        guard let organizationID = currentOrganizationID else {
            return " No organization ID set"
        }
        
        var report = " MANUAL ZONE CREATION:\n\n"
        report += "Organization ID: \(organizationID.prefix(8))...\n"
        
        let container = CKContainer(identifier: "iCloud.com.rheirhome.rheirhomeapp")
        let privateDB = container.privateCloudDatabase
        
        // Step 1: Check account status
        report += "\n1. Checking CloudKit account...\n"
        let accountStatus = try? await container.accountStatus()
        report += "   Account status: \(accountStatus == .available ? "Available" : "Not Available")\n"
        
        if accountStatus != .available {
            report += " Cannot create zone - iCloud account not available\n"
            return report
        }
        
        // Step 2: List existing zones
        report += "\n2. Checking existing zones...\n"
        let existingZones = try? await privateDB.allRecordZones()
        let orgZone = existingZones?.first { $0.zoneID.zoneName == "org-shared-\(organizationID)" }
        
        if let existingZone = orgZone {
            report += "   Zone already exists: \(existingZone.zoneID.zoneName)\n"
            
            // Check if it has sharing
            let hasShare = try? await checkZoneHasShare(existingZone, container: container)
            report += "   Zone has share: \(hasShare != nil ? "" : "")\n"
            
            if hasShare == nil {
                report += "\n3. Creating share for existing zone...\n"
                if let existingZone = existingZone {
                    _ = try await createShareForExistingZone(existingZone, organizationID: organizationID, container: container)
                }
                report += "   Share created successfully\n"
            }
        } else {
            // Step 3: Create new zone
            report += "\n3. Creating new zone...\n"
            let newZone = CKRecordZone(zoneID: CKRecordZone.ID(zoneName: "org-shared-\(organizationID)"))
            let savedZone = try? await privateDB.save(newZone)
            report += "   Zone created: \(savedZone?.zoneID.zoneName)\n"
            
            // Step 4: Create share
            report += "\n4. Creating share for new zone...\n"
            if let savedZone = savedZone {
                _ = try await createShareForExistingZone(savedZone, organizationID: organizationID, container: container)
            }
            report += "   Share created successfully\n"
        ...
        report += "\n5. Testing share URL generation...\n"
        let shareURL = await getOrganizationCloudKitShareURL()
        if let shareURL = shareURL {
            report += "   Share URL generated\n"
            report += "   URL: \(shareURL)\n"
        } else {
            report += "   Could not generate share URL\n"
        }
        
        // Step 6: Enable CloudKit mode
        isUsingCloudKitForOrganizationData = true
        report += "\n6. Enabled CloudKit mode\n"
        
        report += "\n MANUAL ZONE CREATION COMPLETED SUCCESSFULLY!\n"
        
        return report
    }
    
    // MARK: - Labor Hours Migration Methods
    
    /// Check if labor hours need migration to use team member IDs
    var needsLaborHoursMigration: Bool {
        guard let project = selectedProject else { return false }
        
        // Check if any work hours are missing employeeID but have employee name
        return project.loggedHours.contains { workHour in
            workHour.employeeID == nil && !workHour.employee.isEmpty
        }
    }
    
    /// Get labor hours migration status report
    func getLaborHoursMigrationStatus() -> String {
        guard let project = selectedProject else {
            return "No project selected for migration analysis"
        }
        
        let totalHours = project.loggedHours.count
        let hoursWithIDs = project.loggedHours.filter { $0.employeeID != nil }.count
        let hoursNeedingMigration = project.loggedHours.filter { $0.employeeID == nil && !$0.employee.isEmpty }.count
        
        var report = "LABOR HOURS MIGRATION STATUS:\n\n"
        report += "Project: \(project.name)\n"
        report += "Total Work Hours: \(totalHours)\n"
        report += "Hours with Team Member IDs: \(hoursWithIDs)\n"
        report += "Hours needing migration: \(hoursNeedingMigration)\n\n"
        
        if hoursNeedingMigration == 0 {
            report += "✅ All work hours have proper team member IDs\n"
            report += "No migration needed!"
        } else {
            report += "⚠️ \(hoursNeedingMigration) work hours need to be migrated\n"
            report += "Migration will link hours to team member IDs"
        }
        
        return report
    }
    
    /// Migrate labor hours to use team member IDs instead of just names
    func migrateLaborHoursToTeamMemberIDs() {
        guard let selectedProject = selectedProject,
              let projectIndex = organizationProjects.firstIndex(where: { $0.id == selectedProject.id }) else {
            print("❌ No selected project for migration")
            return
        }
        
        var updatedCount = 0
        
        for i in 0..<organizationProjects[projectIndex].loggedHours.count {
            var workHour = organizationProjects[projectIndex].loggedHours[i]
            
            // Skip hours that already have team member IDs
            guard workHour.employeeID == nil, !workHour.employee.isEmpty else { continue }
            
            // Find matching team member by name
            if let teamMember = teamMembers.first(where: { $0.name.lowercased() == workHour.employee.lowercased() }) {
                workHour.employeeID = teamMember.id
                organizationProjects[projectIndex].loggedHours[i] = workHour
                updatedCount += 1
                
                print("✅ Migrated work hour for \(workHour.employee) to team member ID \(teamMember.id)")
            } else {
                print("⚠️ Could not find team member for work hour: \(workHour.employee)")
            }
        }
        ...
        print("✅ Migration completed: \(updatedCount) work hours migrated to team member IDs")
    }
    ...
    // MARK: - Project Assignment Methods
    
    /// Assign a team member to a project
    func assignTeamMemberToProject(_ teamMemberID: String, projectID: String) {
        guard let projectIndex = organizationProjects.firstIndex(where: { $0.id.uuidString == projectID }) else {
            print("❌ Project not found for assignment: \(projectID)")
            return
        }
        
        var project = organizationProjects[projectIndex]
        if !project.assignedUserIDs.contains(teamMemberID) {
            project.assignUser(teamMemberID)
            organizationProjects[projectIndex] = project
            
            // Update selected project if it matches
            if selectedProject?.id.uuidString == projectID {
                selectedProject = project
            }
            
            saveLocalBackup()
            print("✅ Assigned team member \(teamMemberID.prefix(8))... to project \(project.name)")
        } else {
            print("ℹ️ Team member \(teamMemberID.prefix(8))... already assigned to project \(project.name)")
        }
    }
}