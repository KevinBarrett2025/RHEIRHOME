...
// MARK: - Master Organization Settings Tab
    ...
    private func organizationInfoSection(_ organization: Organization) -> some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            VStack(spacing: 8) {
                ...
                infoRow("Local Members", "\(projectVM.teamMembers.filter { $0.hasAppAccess }.count + 1) app users")
            ...
            if let currentOrg = authVM.currentOrg {
                ...
                VStack(alignment: .leading, spacing: 8) {
                    HStack {
                        ...
                        VStack(alignment: .leading, spacing: 4) {
                            Text("Subscription Tier (Testing)")
                                .font(.subheadline)
                                .fontWeight(.medium)
                            Text("Current: \(currentOrg.subscriptionTier.displayName) - $\(String(format: "%.0f", currentOrg.subscriptionTier.monthlyPrice))/month")
                                .font(.caption)
                                .foregroundColor(.secondary)
                        }
                        ...
                        Menu("Change Tier") {
                            ForEach([SubscriptionTier.free, SubscriptionTier.professional, SubscriptionTier.enterprise], id: \.self) { tier in
                    ...
                    VStack(alignment: .leading, spacing: 4) {
                        ...
                        ForEach(currentOrg.subscriptionTier.features.prefix(3), id: \.self) { feature in
                            HStack {
                        ...
                        if currentOrg.subscriptionTier.features.count > 3 {
                            Text("+ \(currentOrg.subscriptionTier.features.count - 3) more features")
                                .font(.caption2)
                                .foregroundColor(.blue)
                        }
                    }
        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            VStack(spacing: 8) {
                NavigationLink(destination: CloudKitDebugView()) {
                    debugLinkRow("CloudKit Debug Console", "icloud.and.arrow.up.fill", .blue, "Test zone creation and sharing")
                }
                
                NavigationLink(destination: CloudKitDataView()) {
                    debugLinkRow("CloudKit Data Browser", "cylinder.fill", .green, "View raw CloudKit records")
                }
                
                debugButton("Fix Team Member Labor Hours", "wrench.and.screwdriver.fill", .purple) {
                ...
                debugButton("Fix Team Member Project Assignments", "person.2.arrow.trianglepath", .blue) {
                    projectVM.fixTeamMemberProjectAssignments()
                    alertMessage = "✅ Fixed team member project assignments!\n\nTeam members who have worked on projects are now properly assigned to them."
                    showingAlert = true
                }
                
                debugButton("Emergency Data Recovery", "cross.case.fill", .red) {
                    Task {
                ...
                debugButton("Show Migration Status", "info.circle.fill", .blue) {
                    alertMessage = projectVM.getLaborHoursMigrationStatus()
                    showingAlert = true
                }
                
                debugButton("Clear Pending Invites", "trash.circle.fill", .orange) {
                ...
                debugButton("Force Zone Recreation", "arrow.triangle.2.circlepath.circle.fill", .red) {
                    Task {
                ...
                debugButton("Nuclear Reset All Data", "exclamationmark.triangle.fill", .red) {
