...
final class ProjectViewModel: ObservableObject {
    ...
    // MARK: - Enterprise Intelligence Access Methods (Phase 2C Enhanced)
    
    /// Get business intelligence report for Company Settings dashboard
    func getBusinessIntelligenceReport() -> String? {
        guard isEnterpriseIntelligenceEnabled else { return nil }
        
        // Get real intelligence data instead of placeholder data
        let intelligenceData = getOrganizationalIntelligenceData()
        let topVendors = getTopVendorsBySpending(limit: 5)
        let topPaymentMethods = getTopPaymentMethodsByUsage(limit: 3)
        ...
        let totalSpending = intelligenceData?["totalSpending"] as? Double ?? organizationProjects.flatMap { $0.receipts }.reduce(0) { $0 + $1.amount }
        ...
        • Total Payment Methods: \(paymentMethodCount)
        • Active Projects: \(organizationProjects.filter { $0.status == .active }.count)
        • Completed Projects: \(organizationProjects.filter { $0.status == .completed }.count)
        • Team Members: \(teamMembers.count)
        
        💰 Financial Overview:
        • Total Organization Spending: $\(String(format: "%.2f", totalSpending))
        • Average Project Budget: $\(organizationProjects.isEmpty ? "0.00" : String(format: "%.2f", organizationProjects.reduce(0) { $0 + $1.totalBudget } / Double(organizationProjects.count)))
        
        """
        
        // Add top vendors from real intelligence data
        if !topVendors.isEmpty {
            report += "🏪 Top Vendors by Spending:\n"
            for (index, vendor) in topVendors.enumerated() {
                report += "• \(index + 1). \(vendor.vendor): $\(String(format: "%.2f", vendor.amount))\n"
            }
            report += "\n"
        }
        
        // Add top payment methods from real intelligence data
        if !topPaymentMethods.isEmpty {
            report += "💳 Most Used Payment Methods:\n"
            for (index, payment) in topPaymentMethods.enumerated() {
                report += "• \(index + 1). \(payment.paymentMethod) (\(payment.count) uses, $\(String(format: "%.2f", payment.amount)))\n"
            }
            report += "\n"
        }
        
        report += "🎯 Intelligence Status: ✅ ACTIVE"
        
        return report
    }
    
    /// Get new project suggestions based on organizational intelligence
    func getNewProjectSuggestions() -> String? {
        guard isEnterpriseIntelligenceEnabled else { return nil }
        
        // Use real intelligence data instead of placeholder data
        let topVendors = getTopVendorsBySpending(limit: 5)
        let topPaymentMethods = getTopPaymentMethodsByUsage(limit: 3)
        ...
        // Smart vendor recommendations based on actual usage
        if !topVendors.isEmpty {
            suggestions += "🏪 Recommended Vendors (Based on Your Organization's Usage):\n"
            for (index, vendor) in topVendors.enumerated() {
                suggestions += "• \(index + 1). \(vendor.vendor) (Total spent: $\(String(format: "%.2f", vendor.amount)))\n"
            }
            suggestions += "\n"
        } else {
            suggestions += "🏪 Recommended Vendors:\n"
            suggestions += "• No vendor data available yet - start adding receipts to build intelligence!\n\n"
        }
        
        // Smart payment method recommendations based on actual usage
        if !topPaymentMethods.isEmpty {
            suggestions += "💳 Your Organization's Preferred Payment Methods:\n"
            for (index, payment) in topPaymentMethods.enumerated() {
                suggestions += "• \(index + 1). \(payment.paymentMethod) (Used \(payment.count) times)\n"
            }
            suggestions += "\n"
        } else {
            suggestions += "💳 Preferred Payment Methods:\n"
            suggestions += "• No payment method data available yet - start adding receipts to build patterns!\n\n"
        }
        
        // Smart category recommendations based on spending patterns
        let intelligenceRecords = getReceiptIntelligenceRecords()
        if !intelligenceRecords.isEmpty {
            var categorySpending: [String: Double] = [:]
            for record in intelligenceRecords {
                if let category = record["category"] as? String,
                   let amount = record["amount"] as? Double {
                    categorySpending[category, default: 0.0] += amount
                }
            }
            
            let topCategories = categorySpending.sorted { $0.value > $1.value }.prefix(3)
            if !topCategories.isEmpty {
                suggestions += "🎯 Your Organization's Top Spending Categories:\n"
                for (index, category) in topCategories.enumerated() {
                    suggestions += "• \(index + 1). \(category.key.capitalized): $\(String(format: "%.2f", category.value))\n"
                }
                suggestions += "\n"
            }
        }
        
        suggestions += "💡 Tip: Keep adding receipts to improve your organizational intelligence and get better recommendations!"
        
        return suggestions
    }
    
    /// Get annual business report for tax season
    func getAnnualBusinessReport(year: Int? = nil) async -> String? {
        guard isEnterpriseIntelligenceEnabled else { return nil }
        
        let reportYear = year ?? Calendar.current.component(.year, from: Date())
        let intelligenceData = getOrganizationalIntelligenceData()
        let totalSpending = intelligenceData?["totalSpending"] as? Double ?? organizationProjects.flatMap { $0.receipts }.reduce(0) { $0 + $1.amount }
        let activeProjects = organizationProjects.filter { $0.status == .active }.count
        let completedProjects = organizationProjects.filter { $0.status == .completed }.count
        
        // Get detailed spending analysis
        let topVendors = getTopVendorsBySpending(limit: 10)
        let topPaymentMethods = getTopPaymentMethodsByUsage(limit: 5)
        
        var report = """
        📋 ANNUAL BUSINESS REPORT (\(reportYear))
        
        💰 Financial Summary:
        • Total Organization Spending: $\(String(format: "%.2f", totalSpending))
        • Average Monthly Spending: $\(String(format: "*.2f", totalSpending / 12.0))
        
        🏗️ Project Summary:
        • Active Projects: \(activeProjects)
        • Completed Projects: \(completedProjects)
        • Total Projects: \(organizationProjects.count)
        
        👥 Team Summary:
        • Total Team Members: \(teamMembers.count)
        • Active Team Members: \(teamMembers.filter { $0.employmentStatus == .active }.count)
        
        📊 Business Intelligence:
        • Vendor Partners: \(vendorService.vendorsSortedByName.count)
        • Payment Methods Used: \(paymentMethodService.paymentMethodsSortedByName.count)
        
        """
        
        // Add detailed vendor analysis
        if !topVendors.isEmpty {
            report += "🏪 Top Vendor Partners by Spending:\n"
            for (index, vendor) in topVendors.enumerated() {
                let percentage = totalSpending > 0 ? (vendor.amount / totalSpending) * 100 : 0
                report += "• \(index + 1). \(vendor.vendor): $\(String(format: "%.2f", vendor.amount)) (\(String(format: "%.1f", percentage))%)\n"
            }
            report += "\n"
        }
        ...
        if !topPaymentMethods.isEmpty {
            report += "💳 Payment Method Usage Analysis:\n"
            for (index, payment) in topPaymentMethods.enumerated() {
                report += "• \(index + 1). \(payment.paymentMethod): \(payment.count) transactions, $\(String(format: "%.2f", payment.amount))\n"
            }
            report += "\n"
        }
        
        report += "✅ Report Generated: \(Date().formatted(date: .abbreviated, time: .shortened))"
        
        return report
    }

    /// Check if enterprise intelligence is initialized and ready
    var isEnterpriseIntelligenceReady: Bool {
        return isEnterpriseIntelligenceEnabled
    }
    
    // MARK: - Phase 2C: Intelligence Activation Methods
    
    /// Trigger historical intelligence population (Phase 2C activation method)
    func activateRealTimeIntelligence() async {
        print("🚀 PHASE 2C: Activating real-time organizational intelligence...")
        
        // Scan existing receipts to populate intelligence
        await scanExistingReceiptsForIntelligence()
        
        print("✅ PHASE 2C ACTIVATION COMPLETE: Real-time organizational intelligence is now LIVE!")
    }
    
    /// Get intelligence status for debugging/monitoring
    func getIntelligenceStatus() -> String {
        guard let orgID = currentOrganizationID else {
        ...
        let totalSpending = intelligenceData?["totalSpending"] as? Double ?? 0.0
        let lastUpdated = intelligenceData?["lastUpdated"] as? Double ?? 0.0
        ...
        Intelligence Records: \(records.count)
        Total Spending Tracked: $\(String(format: "%.2f", totalSpending))
        Last Updated: \(lastUpdatedDate?.formatted(.dateTime.day().month().year().hour().minute()) ?? "Never")
        Status: \(isEnterpriseIntelligenceEnabled ? "✅ ACTIVE" : "❌ INACTIVE")
        
        Ready for Dashboard: \(records.isEmpty ? "❌ Run activateRealTimeIntelligence()" : "✅ YES")
        """
    }
    
    // MARK: - Intelligence Dashboard Support Methods (Phase 2G)
    
    /// Calculate total spent for a specific project (Enterprise Intelligence Method)
    func calculateTotalSpent(for project: Project) -> Double {
        let generalConditionsSpent = project.receipts
            .filter { $0.category == .general }
            .reduce(0) { $0 + ($1.isReturn ? -$1.amount : $1.amount) }
        
        let materialsSpent = project.receipts
            .filter { $0.category == .material }
            .reduce(0) { $0 + ($1.isReturn ? -$1.amount : $1.amount) }
        
        let laborSpent = project.loggedHours
            .reduce(0) { $0 + ($1.hours * $1.rate) }
        
        let contingencySpent = project.receipts
            .filter { $0.category == .contingency }
            .reduce(0) { $0 + ($1.isReturn ? -$1.amount : $1.amount) }
        
        let totalSpent = generalConditionsSpent + materialsSpent + laborSpent + contingencySpent
        
        guard totalSpent.isFinite else {
            return 0
        }
        
        return max(0, totalSpent)
    }
    
    /// Get team members working on a specific project (Enterprise Intelligence Method)
    func getProjectTeamMembers(for project: Project) -> [TeamMember] {
        ...
        let hoursMemberIDs = project.loggedHours.compactMap { $0.employeeID }
        
        // Combine all sets
        let workingMemberIDs = Set(receiptMemberIDs + progressMemberIDs + hoursMemberIDs)
        ...
        return teamMembers.filter { member in
            ...
            workingMemberIDs.contains(member.id)
        }
    }
    ...
    // Vendor service - using the real service (accessible to extensions)
    internal var vendorService: VendorManagementService {
        return _vendorService
    }
    
    // Payment method service - using the real service (accessible to extensions)
    internal var paymentMethodService: PaymentMethodManagementService {
    ...
    func getOrganizationDataMigrationStatus() -> String {
        ...
        let vendorCount = _vendorService.vendorsSortedByName.count
        let paymentMethodCount = _paymentMethodService.paymentMethodsSortedByName.count
    ...
    func fixTeamMemberProjectAssignments() {
        ...
        for i in 0..<organizationProjects.count {
            ...
            let assignedMemberIDs = Set(project.assignedTeamMemberIDs)
            let allRelevantMemberIDs = assignedMemberIDs.union(allWorkingMemberIDs)
            
            // Assign any working members who aren't officially assigned
            for memberID in allRelevantMemberIDs {
    ...
    // MARK: - Analytics Structures (moved here to fix compilation)
    ...
    func getDataStatus() async -> String {
        ...
        - Team Members: \(teamMembers.count)
        - Vendors: \(_vendorService.vendorsSortedByName.count)
        - Payment Methods: \(_paymentMethodService.paymentMethodsSortedByName.count)
