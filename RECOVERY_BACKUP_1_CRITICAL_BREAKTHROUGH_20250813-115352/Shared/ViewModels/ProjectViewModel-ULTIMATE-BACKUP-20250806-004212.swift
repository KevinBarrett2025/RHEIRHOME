...
final class ProjectViewModel: ObservableObject {
    ...
    init(cloudKitService: CloudKitAuthService, useCloudKit: Bool = true) {
        ...
        print(" ProjectViewModel initialized - waiting for organization selection")
    ...
    private func initializeDataSafely() async {
        ...
        print(" INITIALIZING ProjectViewModel data for organization: \(orgID.prefix(8))...")
    ...
    internal func saveLocalBackup() {
        print(" DEPRECATED: saveLocalBackup() called - redirecting to secure method")
        saveOrganizationSpecificBackup()
    }
    ...
    private func loadLocalProjectsAsBackup() async {
        ...
        print(" DEPRECATED: loadLocalProjectsAsBackup() called - redirecting to secure method")
        await loadOrganizationSpecificProjects(organizationID: orgID)
    }
    ...
    private func loadOrganizationProjects() async {
        ...
        if organizationProjects.isEmpty {
            print(" No projects found for organization \(orgID.prefix(8))... - this is normal for new organizations")
        }
    }
    ...
    private func loadOrganizationFromCloudKit(organizationID: String) async {
        ...
        if let data = UserDefaults.standard.data(forKey: key),
           let organization = try? JSONDecoder().decode(Organization.self, from: data) {
            ...
            print("No organization data found for key: \(key)")
    ...
    func migrateLaborHoursToTeamMemberIDs() {
        ...
        if totalMigrated > 0 {
            ...
            print("No labor hours needed migration")
        }
    }
    ...
    func fixTeamMemberProjectAssignments() {
        ...
        if fixedAssignments > 0 {
            ...
            print("No team member assignments needed fixing")
        }
    }
    ...
    /// Recompute labor data for team members
    func recomputeLaborData() {
        print("🔄 Recomputing labor data for \(teamMembers.count) team members")
        
        // Clear existing computations
        groupedHoursByTeamMember.removeAll()
        laborTotalsByTeamMember.removeAll()
        
        // Recompute for each team member
        for teamMember in teamMembers {
            let memberHours = organizationProjects.flatMap { project in
                project.loggedHours.filter { hour in
                    if let employeeID = hour.employeeID {
                        return employeeID == teamMember.id
                    } else {
                        return hour.employee.lowercased() == teamMember.name.lowercased()
                    }
                }
            ...
            groupedHoursByTeamMember[teamMember.name] = memberHours
            
            let unpaidHours = memberHours.filter { !$0.isPaid }.reduce(0) { $0 + ($1.hours * $1.rate) }
            let paidHours = memberHours.filter { $0.isPaid }.reduce(0) { $0 + ($1.hours * $1.rate) }
            
            laborTotalsByTeamMember[teamMember.name] = (unpaid: unpaidHours, paid: paidHours)
        }
        
        print("✅ Labor data recomputed for \(teamMembers.count) team members")
    }
    ...
    func removeProgressLog(_ progressLogID: UUID) {
        ...
        print("Removed progress log from project")
    }
