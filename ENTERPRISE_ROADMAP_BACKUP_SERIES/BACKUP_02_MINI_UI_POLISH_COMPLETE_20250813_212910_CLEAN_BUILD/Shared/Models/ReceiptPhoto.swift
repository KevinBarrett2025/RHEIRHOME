...
/// Represents a receipt photo stored in CloudKit using CKAsset
public struct ReceiptPhoto: Identifiable, Codable, Hashable, Sendable {
    public let id: UUID
...
extension ReceiptPhoto {
    ...
    public func toCKRecord() -> CKRecord {
        let record = CKRecord(recordType: "ReceiptPhoto", recordID: CKRecord.ID(recordName: id.uuidString))
    ...
    public static func fromCKRecord(_ record: CKRecord) -> ReceiptPhoto? {
        guard let receiptIDString = record["receiptID"] as? String,
              let receiptID = UUID(uuidString: receiptIDString),
