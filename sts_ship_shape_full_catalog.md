...
- Simulator build path: `mcp__xcodebuildmcp__build_sim` with `extraArgs=["-derivedDataPath","/tmp/rheir_gateA_phase1by_mcp_dd"]`
- Focused parity path: `mcp__xcodebuildmcp__test_sim` with `extraArgs=["-derivedDataPath","/tmp/rheir_parity_phase1by_mcp_dd","-only-testing:RHEIRTests","-only-testing:RHEIRUITests/RHEIRUITests/testSignedOutModeShowsAppleSignIn","-only-testing:RHEIRUITests/RHEIRUITestsLaunchTests/testLaunch"]`
- Latest focused parity count: `29/29`
...
- `RHEIRUITests` is only deterministic for the signed-out launch path so far; ready-state and organization/project workflow coverage still need dedicated test launch modes.
