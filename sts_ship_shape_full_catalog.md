...
- Simulator build path: `mcp__xcodebuildmcp__build_sim` with `extraArgs=["-derivedDataPath","/tmp/rheir_gateA_phase1bt_mcp_dd"]`
- Focused parity path: `mcp__xcodebuildmcp__test_sim` with `extraArgs=["-derivedDataPath","/tmp/rheir_parity_phase1bt_mcp_dd","-only-testing:RHEIRTests"]`
...
- Standing warning debt remains in `Shared/Features/Receipts/ReceiptScannerView.swift`, `Shared/Views/Settings/PersonalSettingsView.swift`, and `Shared/Models/WorkHour.swift`.
