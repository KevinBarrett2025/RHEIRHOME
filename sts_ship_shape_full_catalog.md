...
- Stable simulator and elevated CLI simulator paths are warning-clean for the active target, and device launch/file-state validation now shows compact `projects.json`, `offline_projects.json`, and prefs payloads; remaining release hardening centers on the simplified single-user contractor flow, scanned-receipt return/reopen, and same-device/device QA.
- `RHEIRUITests` is now deterministic for signed-out, ready-state, fast-ship organization auto-resolution, project-selection, receipts gate/entry, labor gate/empty state, tasks gate/empty state, manual-entry vendor-picker, manual-entry add-vendor, manual-entry payment-method-picker, manual-entry add-payment-method, manual-entry submission, manual-receipt relaunch persistence, saved receipt detail, receipt-detail action, persisted receipt-edit mutation, saved-receipt search/browse, category/filter drilldown, `By Vendor` grouped-summary/expansion coverage, and selected-project AI Project Calculator build/approve plus actual-cost mapping coverage; the next hardening slice is scanned-receipt return/reopen and device QA on this simplified shell rather than more surface reshaping or managed-backend expansion.
...
- Physical-device build/signing is now working and launch/file-state compaction is validated; simulator parity is now also green for CloudKit upsert, duplicate receipt normalization, persisted-edit behavior, scanner host lifecycle, and manual-receipt relaunch persistence, but the fast-ship contractor flow still needs an end-to-end device pass covering sign-in, first scan, scanned-receipt return/reopen, and same-user restore expectations.
