...
- `RHEIRUITests` is now deterministic for signed-out, ready-state, organization-selection, project-selection, labor, company-admin, receipts-entry, manual-entry vendor-picker, manual-entry payment-method-picker, and manual-entry add-payment-method coverage; the next release-hardening slice is the add-vendor route before broader runtime QA.
