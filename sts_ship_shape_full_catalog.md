...
- Simulator build path: `mcp__xcodebuildmcp__build_sim` with `extraArgs=["-derivedDataPath","/tmp/rheir_gateA_phase1ce_mcp_dd"]`
...
- Latest focused parity count: `35/35`
...
- `RHEIRUITests` is now deterministic for signed-out, ready-state, organization-selection, project-selection, labor, company-admin, and receipts-entry coverage; the next release-hardening slice is a deeper selected-project receipt workflow route before broader runtime QA.
