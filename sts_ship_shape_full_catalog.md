...
- Physical-device build/signing is now working and launch/file-state compaction is validated; simulator parity is now also green for CloudKit upsert, duplicate receipt normalization, persisted-edit behavior, and the scanner launch-state lifecycle, but the exact user-driven first-scan receipt mutation path still needs a manual rerun with fresh logs.
