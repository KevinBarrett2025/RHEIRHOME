...
- The latest device freeze root cause was duplicate restored-organization activation in the fast-ship session path, not scanner or receipt persistence; the current code now defers restored-org activation to the session store and ignores duplicate same-org activation work.
- The latest follow-on risk in that seam was synchronous session-store organization activation inside auth publisher callbacks; the current code now waits for org loading to settle and resolves the fast-ship workspace on the next main-actor turn.
