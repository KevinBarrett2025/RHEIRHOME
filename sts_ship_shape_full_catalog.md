...
- Deterministic UI smoke coverage is now in place for the simplified shell plus AI Project Calculator intake/draft/approval and live receipt/hour/task mapping parity into approved budget lines, but broader approved-baseline runtime QA and relaunch/device validation are still open.
...
- Stable simulator and elevated CLI simulator paths are warning-clean for the active target, and device launch/file-state validation now shows compact `projects.json`, `offline_projects.json`, and prefs payloads; remaining release hardening centers on the simplified single-user contractor flow, relaunch persistence, and same-device/device QA.
- `RHEIRUITests` is now deterministic for signed-out, ready-state, fast-ship organization auto-resolution, project-selection, labor, receipts-entry, manual-entry vendor-picker, manual-entry add-vendor, manual-entry payment-method-picker, manual-entry add-payment-method, manual-entry submission, saved receipt detail, receipt-detail action, persisted receipt-edit mutation, saved-receipt search/browse, category/filter drilldown, `By Vendor` grouped-summary/expansion coverage, and selected-project AI Project Calculator build/approve plus actual-cost mapping coverage; the next hardening slice is broader fast-ship v1 UI/UX standardization plus relaunch/runtime QA rather than managed-backend expansion.
...
- Physical-device build/signing is now working and launch/file-state compaction is validated; simulator parity is now also green for CloudKit upsert, duplicate receipt normalization, persisted-edit behavior, and the scanner host lifecycle, but the fast-ship contractor flow still needs an end-to-end device pass covering sign-in, first scan, relaunch restore, and receipt persistence after returning to the receipts surface.
